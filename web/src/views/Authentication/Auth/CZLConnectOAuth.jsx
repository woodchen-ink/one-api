...
// ================================|| AUTH3 - CZLCONNECT OAUTH ||================================ //

const CZLConnectOAuth = () => {
  ...
  const { czlconnectLogin } = useLogin();

  let navigate = useNavigate();

  const sendCode = async (code, state, count) => {
    const { success, message } = await czlconnectLogin(code, state);
    if (!success) {
      if (message) {
        showError(message);
      }
      if (count === 0) {
        setPrompt(t('login.czlconnectError'));
        await new Promise((resolve) => setTimeout(resolve, 2000));
        navigate('/login');
        return;
      }
      count++;
      setPrompt(t('login.czlconnectCountError', { count }));
      await new Promise((resolve) => setTimeout(resolve, 2000));
...
export default CZLConnectOAuth;