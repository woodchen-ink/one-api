...
const SubscriptionPlans = () => {
  ...
  const [purchaseCycle, setPurchaseCycle] = useState('monthly');
  ...
  const handleBuy = (plan, billingCycle) => {
    if (!selectedPayment) {
      showError(t('subscription.selectPayment'));
      return;
    }
    setPurchasePlan(plan);
    setPurchaseCycle(billingCycle);
  ...
  return (
    <Box>
      ...
      <SubscriptionPlanCards plans={plans} loading={loading} onBuy={handleBuy} />
...
const SubscriptionPayDialog = ({ open, onClose, planId, uuid, billingCycle }) => {
  const theme = useTheme();
  ...
  }, [open, planId, uuid, billingCycle, onClose, pollOrderStatus]);
